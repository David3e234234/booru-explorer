...
export async function createSubscription(query, site = 'all') {
  const res = await fetch('/api/subscriptions', {
    method: 'POST',
    headers: getAuthHeaders(true),
...
export async function checkSubscription(id) {
  const res = await fetch(`/api/subscriptions/${encodeURIComponent(id)}/check`, {
    method: 'POST',
    headers: getAuthHeaders(true)
  });
  return await res.json();
...
export async function markSubscriptionSeen(id) {
  const res = await fetch(`/api/subscriptions/${encodeURIComponent(id)}/seen`, {
    method: 'POST',
    headers: getAuthHeaders(true)
  });
  return await res.json();
