...
function getFfmpegHeaders(targetUrl) {
  ...
  try {
    ...
    else if (parsed.hostname.includes('rule34.paheal.net')) referer = 'https://rule34.paheal.net/';
  ...
  return `User-Agent: ${BOORU_USER_AGENT}\r\nReferer: ${referer}\r\n${authHeader}`;
}
...
async function fetchSafe(url, options = {}) {
  ...
  const isBrowserTarget = url.includes('rule34video.com') || url.includes('gelbooru.com') || url.includes('xbooru.com') || url.includes('hypnohub.net');
...
// 4. Rule34 (Paheal открытый XML / API key)
async function fetchRule34(params, aiTagsList, settings) {
  ...
  const pid = Math.max(0, page - 1);
  let url = `https://api.rule34.xxx/index.php?page=dapi&s=post&q=index&json=1&tags=${encodeURIComponent(searchTags)}&pid=${pid}&limit=${limit}`;
  
  if (settings.rule34ApiKey && settings.rule34UserId) {
    url += `&api_key=${encodeURIComponent(settings.rule34ApiKey)}&user_id=${encodeURIComponent(settings.rule34UserId)}`;
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (!text.includes('Missing authentication')) {
        ...
        if (Array.isArray(data) && data.length > 0) {
          return data.map(item => {
            const rawTags = (item.tags || '').split(' ').filter(Boolean);
            const fileUrl = item.file_url || (item.image && item.directory ? `https://us.rule34.xxx/images/${item.directory}/${item.image}` : '');
    ...
    logError('Rule34.xxx', 'API key запрос не удался, переходим на Paheal', err);
  }
  ...
  while ((match = tagRegex.exec(text)) !== null) {
    ...
    if (attrs.file_url) {
      ...
      const { isVideo, isGif, hasSound, fileExt } = checkMediaTypes(attrs.file_url, '', rawTags);
  ...
  // Для "Популярное" на Paheal — показываем самые актуальные трендовые работы
...
app.get('/api/proxy', async (req, res) => {
  ...
  try {
    ...
    const isBrowserTarget = targetUrl.includes('rule34video.com') || targetUrl.includes('gelbooru.com') || targetUrl.includes('xbooru.com') || targetUrl.includes('hypnohub.net');
    const ua = isBrowserTarget ? BROWSER_USER_AGENT : BOORU_USER_AGENT;
    ...
    try {
      const parsed = new URL(targetUrl);
      if (parsed.hostname.includes('rule34.paheal.net')) {
