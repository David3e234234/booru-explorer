...
npm test                     # node --test test/run-all.js (unit + parser + integration)
node --test test/unit/tagHelpers.test.js   # single suite; parser suites live in test/unit/parsers/
```
...
- **No linter.** Verification = `npm test`, plus `node --check <file>` for every touched file (works for ESM backend and `public/js` alike), then boot with `--no-open` and smoke-test endpoints via curl (`/api/posts?site=danbooru&limit=5`, `/api/version`, `/api/cache-info`). Server stdout logs are categorized (`[Search]`, `[Proxy]`, `[FFmpeg]`, ...) via `src/utils/logger.js`. Integration tests (`test/integration/routes.test.js`) boot the app on an ephemeral port with `NODE_ENV=test` - no port conflicts.
...
`.github/workflows/deploy.yml` SSHes to Alwaysdata on every push to `main`/`master`: `git reset --hard origin/main`, `npm install --omit=dev`, restart via `touch tmp/restart.txt` (Passenger). **Pushing main ships to production immediately - CI runs no tests/build - verify before pushing.** A Vercel deployment also exists (`vercel.json`); when `VERCEL` env is set, `isServerless` in `src/config/constants.js` redirects data writes to `os.tmpdir()` and skips `app.listen`.
...
Auth model: the client sends its settings/API keys on every request via the `x-booru-auth` header (see `getAuthHeaders` in `public/js/api.js`); the server merges it over its own `data/settings.json`. Multi-user sessions live in `data/users/`. Secrets list lives in `SECRET_SETTING_FIELDS` (`src/config/constants.js`) - never return those to clients or include them in backups.

Storage: all persistence is JSON files in `data/` (gitignored) with debounced async writes (`writeJsonFileAsync`, 150ms debounce, flushed on SIGINT/SIGTERM/exit in `server.js`). Posts saved to favorites/likes/dislikes must go through `sanitizeStoredPost` (`src/routes/user.routes.js`) which strips heavy nested fields (`albumItems`, `allSeriesKeys`) - keep new bulky fields out of stored payloads.

Backups: outside serverless, `initBackupScheduler` (`src/services/backupService.js`) zips `data/` and sends it to a user-configured Telegram bot.
...
- Visual changes: follow `DESIGN.md` (collector's-cabinet identity, amber `#e5a968` single accent, Fraunces / Plus Jakarta Sans / JetBrains Mono).
