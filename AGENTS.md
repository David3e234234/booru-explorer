...
`.github/workflows/deploy.yml` SSHes to Alwaysdata on every push to `main`/`master`: `git reset --hard origin/main`, `npm install --omit=dev`, restart. **Pushing main ships to production immediately - verify before pushing.** A Vercel deployment also exists (`vercel.json`, live demo linked in README); when `VERCEL` env is set, `isServerless` in `src/config/constants.js` redirects data writes to `os.tmpdir()` and skips `app.listen`.
...
- `.agents/` and `anti-slop/` are local agent tooling: gitignored, never commit them.
