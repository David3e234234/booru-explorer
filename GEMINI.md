...
* **2026-08-16**:
  ...
  * **Рефакторинг фильтра типажей (Мамочки / Лоли)**: Ползунок возраста переориентирован на телосложение и архетипы (`👠 Мамочки` — пышные/зрелые vs `🌸 Лоли` — миниатюрные/плоские). Обновлены словари `CURVY_*` и `PETITE_*`, оптимизированы автозапросы и пост-фильтрация.
  * **Вкладка «Любимые авторы» в Избранном**: Реализована полноценная под-система управления любимыми художниками:
    * Хранение в `data/favorite_authors.json` (исключено из Git в `.gitignore`).
    * Новые API: `GET /api/favorite-authors`, `POST /api/favorite-authors` (toggle), `DELETE /api/favorite-authors/:name`.
    * Под-навигация в разделе Избранное (`🖼️ Посты` и `🎨 Любимые авторы`).
    * Карточки авторов с превью, поиском в 1 клик и удалением.
  * **Фикс видео и превью на Rule34.xxx**:
    * **Разделение sample_url и file_url для видео**: В DAPI Rule34.xxx поле `sample_url` для видеопостов указывает на статический JPEG-эскиз кадра (87 КБ), а само MP4-видео хранится в `file_url`. Ранее плеер и превью ошибочно брали `sample_url` для тега `<video>`, пытаясь проиграть картинку как видео. Исправлен приоритет `fileUrl` для видео в `server.js`, `viewer.js` и `gallery.js`.
    * **Встроенный HTML Scraper для Rule34.xxx**: Поскольку официальный API `api.rule34.xxx` требует обязательную аутентификацию (API key), добавлен автоматический парсер открытой веб-выдачи `rule34.xxx/index.php?page=post&s=list`, который извлекает актуальные посты с корректными `.mp4` ссылками на видео и статическими JPEG-миниатюрами без необходимости вводить API-ключ.
    * **Устранение зацикливания и багов воспроизведения видео**: Скорректированы заголовки `Referer` (`https://rule34.xxx/`, `https://rule34.paheal.net/`) и `User-Agent` в `/api/proxy` и `getFfmpegHeaders()`, нормализован дублирующийся заголовок `Content-Type` от Paheal CDN.
    * **Корректные превью в галерее**: Устранена ошибка в `gallery.js`, из-за которой видеофайлы без явного расширения ошибочно подставлялись в тег `<img>` карточки вместо статического эскиза или генератора превью `/api/video-thumbnail`.
